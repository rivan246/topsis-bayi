 ...
 * ------------------------------------------- */
$query2 = $pdo->prepare('SELECT id_kambing, ciri_khas,no_kalung FROM kambing');
$query2->execute();			
...
foreach($kambings as $kambing):
	...
	$ranks[$kambing['id_kambing']]['ciri_khas'] = $kambing['ciri_khas'];
...
<div class="container clearfix">	
	...
	<div class="main-content main-content-full the-content">
		...
		<table class="pure-table pure-table-striped">
			<thead>					
				<tr>
					<th class="super-top-left">Nomor</th>
		...
		if(function_exists('array_multisort')):
			...
			array_multisort($nilai, SORT_DESC,$ciri_khas, SORT_REGULAR, $no_kalung, SORT_ASC, $sorted_ranks);
		endif;
		?>		
		...
		<table class="pure-table pure-table-striped">
			...
			<tbody>
				<?php foreach($sorted_ranks as $kambing ): ?>
					<tr>
						<td><?php echo $kambing['no_kalung']; ?></td>
						<td><?php echo $kambing['ciri_khas']; ?></td>
